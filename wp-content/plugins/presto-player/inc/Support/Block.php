...
?>
    ...
    public function getFeedHtml($atts)
    {
        if (is_feed()) {
            ...
            if (in_array($atts['provider'], array('self-hosted', 'bunny'))) {

                ob_start();
        ?>
                <video controls preload="none">
                    <source src="<?php echo esc_attr($atts['src']); ?>" />
                </video>
            <?php
                return ob_get_clean();
            } else if ($atts['provider'] == 'youtube') {

                ob_start();
            ?>
                <div class="presto-iframe-fallback-container">
                    <iframe style="width: 100%" class="presto-fallback-iframe" id="presto-iframe-fallback-<?php echo (int) $atts['id']; ?>" src="https://www.youtube.com/embed/<?php echo esc_attr($atts['id']); ?>"></iframe>
                </div>
            <?php
                return ob_get_clean();
            } else {
            ...
            ?>
                <div class="presto-iframe-fallback-container">
                    <iframe style="width: 100%" class="presto-fallback-iframe" id="presto-iframe-fallback-<?php echo (int) $atts['id']; ?>" src="https://player.vimeo.com/video/<?php echo esc_attr($atts['id']); ?>"></iframe>
                </div>
<?php
                return ob_get_clean();
            }
